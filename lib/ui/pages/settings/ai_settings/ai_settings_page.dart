...
class AiSettingsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => AISettingsViewModel(context.read(), context.read()),
      child: Builder(
        builder: (context) {
          ...
          final explainPrompt = context.select(
            (AISettingsViewModel vm) => vm.aiPrompts.explainPrompt,
          );
          final translatePrompt = context.select(
            (AISettingsViewModel vm) => vm.aiPrompts.translatePrompt,
          );
                  ...
                  child: SingleChildScrollView(
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          // AI Provider
                          TitleText(AppLocalizations.of(context)!.aiProvider),
                          ...
                          // AI Model
                          TitleText(AppLocalizations.of(context)!.aiModel),
                          const SizedBox(height: 12),
                          const ModelSetting(),
                          const SizedBox(height: 24),

                          // API Key
                          TitleText(AppLocalizations.of(context)!.apiKey),
                          const SizedBox(height: 12),
                          const APIKeySetting(),
                          const SizedBox(height: 24),
                          ...
                          PromptSetting(
                            ...
                            onChanged:
                                viewModel.aiPrompts.setWritingCheckPrompt,
                            onReset:
                                viewModel.aiPrompts.resetWritingCheckPrompt,
                          ),
                        ],
                      ),
                    ),
